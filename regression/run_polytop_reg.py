#!/usr/bin/env python
"""
A command line interface to give an example how to use the CPT module.
"""
...
if __name__ == '__main__':
  ...
  parser.add_argument("--data", default='MNIST', type=str, 
                      choices=['MNIST','FashionMNIST','protein','SensIT',
                               'connect4', 'bace', 'HIV', 'sider', 'tox21', 'PDBbind'],#,'ISBI'],
  ...
  for depth in args.depths:
    ...
    model.eval()
    train_score_discrete = compute_rmse(model, train_set, cuda=args.cuda)
    valid_score_discrete = compute_rmse(model, valid_set, cuda=args.cuda)
    test_score_discrete = compute_rmse(model, test_set, cuda=args.cuda)
    ...
    if args.refine:
      ...
      for steepness in steepnesses:
        ...
        ref_model.eval()
        train_score_discrete = compute_rmse(ref_model, train_set, cuda=args.cuda)
        valid_score_discrete = compute_rmse(ref_model, valid_set, cuda=args.cuda)
        test_score_discrete = compute_rmse(ref_model, test_set, cuda=args.cuda)
