...
if __name__ == '__main__':
  ...
  parser.add_argument("--data", default='MNIST', type=str, 
                      choices=['MNIST','FashionMNIST','protein','SensIT',
                               'connect4', 'bace', 'HIV', 'sider', 'tox21','Letter'],#,'ISBI'],
  ...
  for depth in args.depths:
    ...
    model.eval()
    train_score_discrete, n_leafs = compute_score(model, train_set, cuda=args.cuda, return_n_leafs=True)
    #valid_score_discrete = compute_score(model, test_set, cuda=args.cuda)
    valid_score_discrete = train_score_discrete
    test_score_discrete = compute_score(model, test_set, cuda=args.cuda)
    ...
    append_list_as_row(args.data+'results.csv',[greedy_scores[-1][0], greedy_scores[-1][1],
                 greedy_scores[-1][2], greedy_scores[-1][3], n_leafs ])
    ...
    if args.refine:
      ...
      for steepness in steepnesses:
        ...
        ref_model.eval()
        train_score_discrete, n_leafs = compute_score(ref_model, train_set, cuda=args.cuda, return_n_leafs=True)
        #valid_score_discrete = compute_score(ref_model, test_set, cuda=args.cuda)
        valid_score_discrete = train_score_discrete
        test_score_discrete = compute_score(ref_model, test_set, cuda=args.cuda)
        ...
        append_list_as_row(args.data+'results.csv',[ref_scores[steepness][-1][0],ref_scores[steepness][-1][1],
                ref_scores[steepness][-1][2],ref_scores[steepness][-1][3], n_leafs])
